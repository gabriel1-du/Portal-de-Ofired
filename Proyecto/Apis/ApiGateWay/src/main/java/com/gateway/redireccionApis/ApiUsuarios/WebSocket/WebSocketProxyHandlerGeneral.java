...
@Component
public class WebSocketProxyHandlerGeneral extends AbstractWebSocketHandler implements SubProtocolCapable {
    ...
    @Value("${services.general.websocket.base-url}")
    private String wsBaseUrl;

    @Value("${services.general.websocket.base-path}")
    ...
    public void afterConnectionEstablished(WebSocketSession frontendSession) throws Exception {
        ...
        String originalPath = frontendSession.getUri().getRawPath().replace("/api/proxy/general-webApi", "");
        String query = frontendSession.getUri().getQuery();
        
        // Construimos la URL objetivo (forzando ws://) + el base path (/ws-api) + el resto
        ...
        System.out.println("WEBSOCKET Proxy General redirigiendo a: " + targetUri);
        ...
        // Creamos el cliente que escuchará lo que responde el microservicio
        ...
        try {
            ...
            System.err.println("Error conectando con el backend WebSocket General: " + e.getMessage());
