package com.gateway.redireccionApis.ApiUsuarios.TipoContenidoDenunciado;
...
@RestController
@RequestMapping("/api/proxy/tipos-contenido-denunciadoApi")
@RequiredArgsConstructor
public class TipoContenidoDenunciadoProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.tipos.contenido.denunciado.base-url}")
    private String tiposContenidoBaseUrl;

    @Value("${services.tipos.contenido.denunciado.base-path}")
    private String tiposContenidoBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET})
    public ResponseEntity<?> proxyTiposContenidoPublic(HttpServletRequest request,
    ...
    public ResponseEntity<?> proxyTiposContenidoSecure(HttpServletRequest request,
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/tipos-contenido-denunciadoApi", "");
        String queryString = request.getQueryString();

        var uriBuilder = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(tiposContenidoBaseUrl)
                .path(tiposContenidoBasePath)
                .path(originalPath);
        ...
        System.out.println("TIPO CONTENIDO DENUNCIADO targetUrl: " + targetUrl + "  METHOD: " + method);
