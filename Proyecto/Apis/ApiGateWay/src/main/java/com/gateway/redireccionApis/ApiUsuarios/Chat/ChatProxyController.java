package com.gateway.redireccionApis.ApiUsuarios.Chat;
...
@RestController
@RequestMapping("/api/proxy/chatApi")
...
public class ChatProxyController {
    ...
    @Value("${services.chat.base-url}")
    private String chatBaseUrl;

    @Value("${services.chat.base-path}")
    private String chatBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE})
    public ResponseEntity<?> proxyChat(HttpServletRequest request,
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/chatApi", "");
        String queryString = request.getQueryString();

        var uriBuilder = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(chatBaseUrl)
                .path(chatBasePath)
                .path(originalPath);
        ...
        System.out.println("CHAT targetUrl: " + targetUrl + "  METHOD: " + method);
