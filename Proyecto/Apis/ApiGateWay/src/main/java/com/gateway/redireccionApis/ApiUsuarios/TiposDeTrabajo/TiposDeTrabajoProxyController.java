package com.gateway.redireccionApis.ApiUsuarios.TiposDeTrabajo;
...
@RestController
@RequestMapping("/api/proxy/tipos-trabajoApi")
@RequiredArgsConstructor
public class TiposDeTrabajoProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.tipos.trabajo.base-url}")
    private String tiposTrabajoBaseUrl;

    @Value("${services.tipos.trabajo.base-path}")
    private String tiposTrabajoBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET})
    public ResponseEntity<?> proxyTiposTrabajoPublic(HttpServletRequest request,
    ...
    public ResponseEntity<?> proxyTiposTrabajoSecure(HttpServletRequest request,
                                                 @RequestBody(required = false) String body,
                                                 @RequestHeader HttpHeaders headers) {
        return handleProxy(request, body, headers);
    }
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/tipos-trabajoApi", "");

        String targetUrl = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(tiposTrabajoBaseUrl)
                .path(tiposTrabajoBasePath)
                .path(originalPath)
                .build(true)
                .toUriString();

        HttpMethod method = HttpMethod.valueOf(request.getMethod());
        System.out.println("TIPOS_TRABAJO targetUrl: " + targetUrl + "  METHOD: " + method);
