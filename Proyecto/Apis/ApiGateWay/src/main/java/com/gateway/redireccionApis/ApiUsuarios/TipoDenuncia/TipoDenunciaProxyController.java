package com.gateway.redireccionApis.ApiUsuarios.TipoDenuncia;
...
@RestController
@RequestMapping("/api/proxy/tipos-denunciaApi")
@RequiredArgsConstructor
public class TipoDenunciaProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.tipos.denuncia.base-url}")
    private String tiposDenunciaBaseUrl;

    @Value("${services.tipos.denuncia.base-path}")
    private String tiposDenunciaBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET})
    public ResponseEntity<?> proxyTiposDenunciaPublic(HttpServletRequest request,
    ...
    public ResponseEntity<?> proxyTiposDenunciaSecure(HttpServletRequest request,
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/tipos-denunciaApi", "");
        String queryString = request.getQueryString();

        var uriBuilder = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(tiposDenunciaBaseUrl)
                .path(tiposDenunciaBasePath)
                .path(originalPath);
        ...
        System.out.println("TIPO DENUNCIA targetUrl: " + targetUrl + "  METHOD: " + method);
