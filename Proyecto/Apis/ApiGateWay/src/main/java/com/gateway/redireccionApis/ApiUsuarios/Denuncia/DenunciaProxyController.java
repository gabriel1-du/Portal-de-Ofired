package com.gateway.redireccionApis.ApiUsuarios.Denuncia;
...
@RestController
@RequestMapping("/api/proxy/denunciasApi")
@RequiredArgsConstructor
public class DenunciaProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    // Inyección de tus nuevas propiedades asignadas a denuncias en application.properties
    @Value("${services.denuncias.base-url}")
    private String denunciaBaseUrl;

    @Value("${services.denuncias.base-path}")
    private String denunciaBasePath;

    // Permiso Público: El método GET no valida roles (Ver denuncias públicas, si aplica)
    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET})
    public ResponseEntity<?> proxyDenunciaPublic(HttpServletRequest request,
                                                 @RequestHeader HttpHeaders headers) {
        return handleProxy(request, null, headers);
    }

    // Permiso Seguro: Métodos de mutación exigen validación de Token y Rol (Crear/Resolver denuncias)
    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE})
    public ResponseEntity<?> proxyDenunciaSecure(HttpServletRequest request,
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        // Limpiamos el prefijo del Gateway para obtener la ruta interna del microservicio
        String originalPath = request.getRequestURI().replace("/api/proxy/denunciasApi", "");
        String queryString = request.getQueryString();

        var uriBuilder = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(denunciaBaseUrl)
                .path(denunciaBasePath)
                .path(originalPath);
        ...
        System.out.println("DENUNCIA targetUrl: " + targetUri + "  METHOD: " + method);

        // Validación de Token y Rol para peticiones de escritura
        ...
        // Extracción y propagación segura del ID de usuario al microservicio de destino
        ...
        if (authHeaderForUserId != null && authHeaderForUserId.startsWith("Bearer ")) {
            try {
                ...
                if (userId != null) {
                    ...
                    System.out.println("-> Propagando cabecera X-User-ID: " + userId + " al servicio de denuncias.");
                }
            } catch (Exception e) {
        ...
        // Previene errores de Content-Type vacíos en métodos de tipo GET
