package com.gateway.redireccionApis.ApiUsuarios.SexoUsuario;
...
@RestController
@RequestMapping("/api/proxy/sexosApi")
@RequiredArgsConstructor
public class SexoUsuarioProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.sexousuario.base-url}")
    private String sexoUsuarioBaseUrl;

    @Value("${services.sexousuario.base-path}")
    private String sexoUsuarioBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET})
    public ResponseEntity<?> proxySexoUsuarioPublic(HttpServletRequest request,
    ...
    public ResponseEntity<?> proxySexoUsuarioSecure(HttpServletRequest request,
                                                 @RequestBody(required = false) String body,
                                                 @RequestHeader HttpHeaders headers) {
        return handleProxy(request, body, headers);
    }
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/sexosApi", "");

        String targetUrl = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(sexoUsuarioBaseUrl)
                .path(sexoUsuarioBasePath)
                .path(originalPath)
                .build(true)
                .toUriString();

        HttpMethod method = HttpMethod.valueOf(request.getMethod());
        System.out.println("SEXO_USUARIO targetUrl: " + targetUrl + "  METHOD: " + method);
