package com.gateway.redireccionApis.ApiUsuarios.MediosDePago;
...
@RestController
@RequestMapping("/api/proxy/medios-pagoApi")
@RequiredArgsConstructor
public class MediosDePagoProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.medios.pago.base-url}")
    private String mediosPagoBaseUrl;

    @Value("${services.medios.pago.base-path}")
    private String mediosPagoBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET})
    public ResponseEntity<?> proxyMediosPagoPublic(HttpServletRequest request,
    ...
    public ResponseEntity<?> proxyMediosPagoSecure(HttpServletRequest request,
                                                 @RequestBody(required = false) String body,
                                                 @RequestHeader HttpHeaders headers) {
        return handleProxy(request, body, headers);
    }
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/medios-pagoApi", "");

        String targetUrl = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(mediosPagoBaseUrl)
                .path(mediosPagoBasePath)
                .path(originalPath)
                .build(true)
                .toUriString();

        HttpMethod method = HttpMethod.valueOf(request.getMethod());
        System.out.println("MEDIOS_PAGO targetUrl: " + targetUrl + "  METHOD: " + method);
