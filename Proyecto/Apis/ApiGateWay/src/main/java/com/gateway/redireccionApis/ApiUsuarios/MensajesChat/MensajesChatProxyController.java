...
@RestController
@RequestMapping("/api/proxy/mensajes-chatApi")
@RequiredArgsConstructor
public class MensajesChatProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.mensajes.chat.base-url}")
    private String mensajesChatBaseUrl;

    @Value("${services.mensajes.chat.base-path}")
    private String mensajesChatBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE})
    public ResponseEntity<?> proxyMensajesChat(HttpServletRequest request,
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/mensajes-chatApi", "");
        String queryString = request.getQueryString();

        var uriBuilder = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(mensajesChatBaseUrl)
                .path(mensajesChatBasePath)
                .path(originalPath);
        ...
        System.out.println("MENSAJES CHAT targetUrl: " + targetUrl + "  METHOD: " + method);
